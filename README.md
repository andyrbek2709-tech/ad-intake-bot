...
**Инициализация Knowledge Base:**
```bash
npm run init:kb  # Загружает примеры услуг и FAQ в базу
```
...
### Knowledge Base (НОВОЕ — включено!)

- **RAG-lite интеграция** — бот ищет в базе знаний relev информацию о услугах, ценах, процессах
- **Автоматический контекст в промп** — AI использует данные KB при ответе клиентам
- **Менеджер-команды:**
  - **`/kb_search <запрос>`** — поиск в базе знаний (для менеджера)
  - **`/kb_add`** — добавить новую запись в KB
  - **`/kb_list`** — показать все записи
- **Инициализация:** `npm run init:kb` — загружает примеры услуг, FAQ, рекомендации
- **Использование:** KB автоматически подмешивается в системный промп, AI использует как рекомендации
